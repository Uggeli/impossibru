...
const entries = (value:any):[string,any][] => Object.entries(value) as [string,any][];
const values = (value:any):any[] => Object.values(value) as any[];
...
function renderPalette() {
  ...
  entries(state.doc.palette).forEach(([char,value])=>{
...
function renderPreviewControls() {
  ...
  entries(state.doc.animations||{}).forEach(([name,spec])=>{const o=document.createElement('option');o.value=name;o.textContent=name;clip.append(o);});
...
function renamePart(){const old=state.part,name=prompt('Rename part',old);if(!name||name===old)return;if(!validName(name)||state.doc.parts[name])return alert('Choose a unique valid name.');checkpoint();const replaced={};entries(state.doc.parts).forEach(([key,value])=>replaced[key===old?name:key]=value);state.doc.parts=replaced;values(state.doc.rig?.bones||{}).forEach(b=>{if(b.part===old)b.part=name;});state.part=name;changed();}
function deletePart(){if(Object.keys(state.doc.parts).length===1)return alert('A project must contain at least one part.');const refs=entries(state.doc.rig?.bones||{}).filter(([,b])=>b.part===state.part).map(([n])=>n);const detail=refs.length?` It will detach bones: ${refs.join(', ')}.`:'';if(!confirm(`Delete ${state.part}?${detail}`))return;checkpoint();refs.forEach(n=>delete state.doc.rig.bones[n].part);delete state.doc.parts[state.part];state.part=Object.keys(state.doc.parts)[0];changed();}
...
function renameColor(old){const char=prompt('Rename palette character',old);if(!char||char===old)return;if(char.length!==1||char==='.'||state.doc.palette[char]!==undefined)return alert('Choose one unused character other than .');checkpoint();const palette={};entries(state.doc.palette).forEach(([k,v])=>palette[k===old?char:k]=v);state.doc.palette=palette;values(state.doc.parts).forEach(part=>['front','back','side'].forEach(view=>part[view]=part[view].split(old).join(char)));state.color=char;changed();}
function deleteColor(char){const used=values(state.doc.parts).some(part=>['front','back','side'].some(view=>part[view].includes(char)));if(used)return alert(`Color ${char} is still used by a part. Erase or replace it first.`);if(Object.keys(state.doc.palette).length===1)return alert('A project needs at least one color.');if(!confirm(`Delete palette color ${char}?`))return;checkpoint();delete state.doc.palette[char];if(state.color===char)state.color=Object.keys(state.doc.palette)[0];changed();}
...
function boneChildren(name){return entries(state.doc.rig.bones).filter(([,b])=>b.parent===name).map(([n])=>n);}
function appendBoneTree(parent,container,depth=0){entries(state.doc.rig.bones).filter(([,b])=>(b.parent||null)===(parent||null)).forEach(([name])=>{const b=document.createElement('button');b.style.paddingLeft=(10+depth*14)+'px';b.textContent=name;b.classList.toggle('active',name===state.selectedBone);b.onclick=()=>{state.selectedBone=name;state.selectedChain=null;renderRig();schedulePreview(0);};container.append(b);appendBoneTree(name,container,depth+1);});}
...
function allTracks(clip){return [...values(clip.bones||{}),...values(clip.ik||{})].flatMap(value=>values(value));}
...
function renameMapKey(map,old,name){const out={};entries(map).forEach(([k,v])=>out[k===old?name:k]=v);return out;}
function addBone(){const name=prompt('New bone name',uniqueObjectName(state.doc.rig.bones,'bone'));if(!name||!validName(name)||state.doc.rig.bones[name])return;checkpoint();const spec:any={translation:[0,1,0]};if(state.selectedBone)spec.parent=state.selectedBone;state.doc.rig.bones[name]=spec;state.selectedBone=name;changed();}
function renameBone(){const old=state.selectedBone,name=prompt('Rename bone',old);if(!name||name===old||!validName(name)||state.doc.rig.bones[name])return;checkpoint();state.doc.rig.bones=renameMapKey(state.doc.rig.bones,old,name);values(state.doc.rig.bones).forEach(b=>{if(b.parent===old)b.parent=name;});values(state.doc.rig.ik_chains||{}).forEach(c=>['root','mid','end'].forEach(k=>{if(c[k]===old)c[k]=name;}));values(state.doc.animations).forEach(c=>{if(c.bones?.[old])c.bones=renameMapKey(c.bones,old,name);});state.selectedBone=name;changed();}
...
function renameChain(){const old=state.selectedChain;if(!old)return;const name=prompt('Rename IK chain',old);if(!name||name===old||!validName(name)||state.doc.rig.ik_chains[name])return;checkpoint();state.doc.rig.ik_chains=renameMapKey(state.doc.rig.ik_chains,old,name);values(state.doc.animations).forEach(c=>{if(c.ik?.[old])c.ik=renameMapKey(c.ik,old,name);});state.selectedChain=name;changed();}
function deleteChain(){const name=state.selectedChain;if(!name)return;const refs=entries(state.doc.animations).filter(([,c])=>c.ik?.[name]).map(([n])=>n);if(refs.length)return alert('Remove IK tracks first: '+refs.join(', '));checkpoint();delete state.doc.rig.ik_chains[name];state.selectedChain=null;changed();}
...
function drawOverlay(){const svg=$('rigOverlay'),g=state.overlay;if(!g)return;svg.setAttribute('viewBox',`0 0 ${g.width} ${g.height}`);svg.replaceChildren();const byName=Object.fromEntries(g.bones.map(b=>[b.name,b])),chainLinks={};g.chains.forEach(c=>{chainLinks[c.mid]=c.color;chainLinks[c.end]=c.color;});g.bones.forEach(b=>{if(!b.parent)return;const p=byName[b.parent],line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',String(p.x));line.setAttribute('y1',String(p.y));line.setAttribute('x2',String(b.x));line.setAttribute('y2',String(b.y));line.setAttribute('class','rig-link'+(chainLinks[b.name]?' chain':''));if(chainLinks[b.name])line.setAttribute('stroke',chainLinks[b.name]);svg.append(line);});g.chains.forEach(c=>{[c.target,c.pole].forEach((h,i)=>{if(!h)return;const circle=document.createElementNS('http://www.w3.org/2000/svg','circle');circle.setAttribute('cx',String(h.x));circle.setAttribute('cy',String(h.y));circle.setAttribute('r',String(i?2:2.5));circle.setAttribute('fill',c.color);circle.setAttribute('class','rig-handle');circle.onclick=()=>{state.selectedChain=c.name;state.selectedTrack=i?'pole':'target';renderRig();};bindRigDrag(circle,i?'pole':'target',c.name,h.depth);svg.append(circle);});});g.bones.forEach(b=>{const circle=document.createElementNS('http://www.w3.org/2000/svg','circle');circle.setAttribute('cx',String(b.x));circle.setAttribute('cy',String(b.y));circle.setAttribute('r',String(b.name===state.selectedBone?2.3:1.7));circle.setAttribute('class','rig-joint'+(b.name===state.selectedBone?' selected':''));circle.onclick=()=>{state.selectedBone=b.name;state.selectedChain=null;renderRig();};bindRigDrag(circle,'joint',b.name,b.depth);svg.append(circle);const text=document.createElementNS('http://www.w3.org/2000/svg','text');text.setAttribute('x',String(b.x+2.5));text.setAttribute('y',String(b.y-2));text.setAttribute('class','rig-label');text.textContent=b.name;svg.append(text);});}
function bindRigDrag(element,kind,name,depth){element.onpointerdown=e=>{e.preventDefault();e.stopPropagation();checkpoint();const pointerId=e.pointerId,dragStart=structuredClone(state.doc);element.setPointerCapture(pointerId);let pending=false,latest=null;const apply=async()=>{if(pending||!latest)return;pending=true;const screen=latest;latest=null;try{const body=await(await request('/api/rig/drag',{method:'POST',body:JSON.stringify({document:dragStart,kind,name,screen,depth,direction:+$('yaw').value,mode:state.workspace==='animate'?'animate':'rest',clip:state.selectedClip,frame:+$('frame').value})})).json();state.doc=body.document;state.dirty=true;state.valid=true;renderBoneInspector();renderTimeline();await updateRigPreview(false);}catch(error){showError(error.message);}finally{pending=false;if(latest)apply();}};element.onpointermove=move=>{if(!element.hasPointerCapture(move.pointerId))return;const rect=$('rigOverlay').getBoundingClientRect(),screen=[(move.clientX-rect.left)*state.overlay.width/rect.width,(move.clientY-rect.top)*state.overlay.height/rect.height];latest=[screen[0]-(state.overlay.offset?.[0]||0),screen[1]-(state.overlay.offset?.[1]||0)];element.setAttribute('cx',String(screen[0]));element.setAttribute('cy',String(screen[1]));apply();};const finish=async()=>{element.onpointermove=null;element.onpointerup=null;element.onpointercancel=null;if(element.hasPointerCapture(pointerId))element.releasePointerCapture(pointerId);while(pending||latest)await new Promise(resolve=>setTimeout(resolve,10));changed();await updateRigPreview(true);};element.onpointerup=finish;element.onpointercancel=finish;};}
