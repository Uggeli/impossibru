...
async function request(path, options={}) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(body.error?.message || 'Request failed'); error.body=body; error.status=response.status; throw error;
  }
  return response;
}
...
window.addEventListener('keydown',event=>{if(['INPUT','SELECT','TEXTAREA'].includes(event.target.tagName))return;const key=event.key.toLowerCase();if((event.ctrlKey||event.metaKey)&&key==='s'){event.preventDefault();save();}else if((event.ctrlKey||event.metaKey)&&key==='z'){event.preventDefault();event.shiftKey?redo():undo();}else if((event.ctrlKey||event.metaKey)&&key==='y'){event.preventDefault();redo();}else {const map={p:'pencil',e:'eraser',f:'fill',i:'eyedropper'};if(map[key])document.querySelector(`[data-tool=${map[key]}]`).click();}});
